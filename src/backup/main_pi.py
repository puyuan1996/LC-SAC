...
from latent_encoder import RecurrentLatentEncoder, RecurrentLatentEncoderSmall
from agent_z2 import Agent

import gym
from spinup.algos.pytorch.sac.core import SquashedGaussianMLPActor, MLPQFunction
from spinup.algos.pytorch.sac.sac_pi_v1 import SAC
...
def run(args):
    ...
    recurrent = True
    latent_encoder_hidden_dim = 128  # 128
    args.cuda = not args.no_cuda and torch.cuda.is_available()
    device = torch.device("cuda" if args.cuda else "cpu")
    args.device = 'cpu'  # device

    if recurrent:
        latent_encoder = RecurrentLatentEncoderSmall(input_dim=latent_encoder_input_dim, latent_dim=latent_dim,
    ...
    algorithm = SAC(env, args.env, agent, q1_net, q2_net,
                    ...
                    seq_len=args.seq_len,batch_size=args.latent_batch_size,
                    save_freq=args.save_freq, model_path=args.model_path, device=args.device,
                    train_steps=args.train_steps, collect_data_samples=args.collect_data_samples, latent_encoder_update_frequency=args.latent_encoder_update_frequency,
                    args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env', type=str, default='LunarLanderContinuous-v2')  # BipedalWalker-v3' MountainCarContinuous-v0' HalfCheetah-v2'
    ...
    parser.add_argument('--model_path', type=str, default='./model_samll/')
    ...
    parser.add_argument('--update_begin_steps', type=int, default=1000)#1000
    parser.add_argument('--latent_encoder_update_frequency', metavar='luf', type=int, default=100)
    parser.add_argument('--seq_len', type=int, default=20)
    parser.add_argument('--latent_batch_size', type=int, default=100)
    parser.add_argument('--env_id_list', type=list, default=[2, 0, 1] + list(range(3, 12)))
    parser.add_argument('--deterministic', type=bool, default=False)
    args = parser.parse_args()
    # args.env_id_list = [6]

    for j in range(5):
        for i in [2, 0, 1] + list(range(3, 12)):
        # for i in args.env_id_list:
            torch.manual_seed(j)
