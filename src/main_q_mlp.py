...
from latent_encoder import MLPLatentEncoder, RecurrentLatentEncoder, RecurrentLatentEncoder2head, RecurrentLatentEncoderDet
from agent_mlp import Agent

import gym
from spinup.algos.pytorch.sac.core import SquashedGaussianMLPActor, MLPQFunction
from spinup.algos.pytorch.sac.sac_q_mlp import SAC
...
def run(args):
    ...
    if recurrent:
        ...
        latent_encoder = MLPLatentEncoder(input_dim=latent_encoder_input_dim, latent_dim=latent_dim,
                                          hidden_dim=latent_encoder_hidden_dim, device=args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env', type=str,
                        default='HalfCheetah-v2')#Hopper-v2')Striker-v2')  # BipedalWalker-v3' MountainCarContinuous-v0' HalfCheetah-v2'
    ...
    parser.add_argument('--model_path', type=str, default='./model_q_MLP/')
    ...
    parser.add_argument('--random_steps', type=int, default=10)  # 10000
    ...
    parser.add_argument('--latent_fq', type=int, default=5)  #5
    ...
    parser.add_argument('-rl_uf', '--rl_update_frequency', type=int, default=50)#1000)  # 1e4

    parser.add_argument('-latent_bs','--latent_buffer_size', type=int, default=1000000)#5000) #50000
    parser.add_argument('-rl_bs','--rl_buffer_size', type=int, default=1000000)#5000)#5000
    ...
    print(f'latent_uf:{args.latent_encoder_update_frequency},rl_uf:{args.rl_update_frequency}')
