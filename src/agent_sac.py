...
class Agent(nn.Module):
    ...
    def forward(self, context_seq_batch):
        ...
        self.z = torch.stack((torch.stack([self.z_mu[2 * i] for i in range(self.latent_batch_size)]), torch.stack([self.z_mu[2 * i+1] for i in range(self.latent_batch_size)])))

        self.z = torch.zeros(2, 100, self.latent_dim)  # TODO

        self.z_mu = torch.stack([self.z_mu[2 * i] for i in range(self.latent_batch_size)]) # 100   # 只提取出 z_t 对应的均值方差，用于计算KL散度
        ...
        # o_z = torch.cat([obs, self.z[0].detach()], dim=1)
        # o_z = torch.cat([obs, self.z[0]], dim=1)
        # # policy_outputs = self.pi(in_, reparameterize=True, return_log_prob=True)
        # pi_action, logp_pi = self.pi(o_z, deterministic=False, with_logprob=True)

        return self.z

    def log(self):
        z_mu = np.mean(np.abs(ptu.get_numpy(self.z_mu[0])))
        z_logvar = np.mean(ptu.get_numpy(self.z_logvar[0]))
