...
class SAC(object):
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch, update_latent_encoder):
        ...
        if self.total_latent_train_steps % 1000 == 0:
            # print('loss_q', loss_q.item())
            print('loss_pi', loss_pi.item())
    ...
    def sac_update_step(self):
        ...
        # Record things
        # self.logger.store(LossQ=loss_q.item(), **q_info)
        self.writer.add_scalar('loss/LossQ', float(loss_q.item()), self.total_train_steps)
        ...
        # Record things
        # self.logger.store(LossPi=loss_pi.item(), **pi_info)
        self.writer.add_scalar('loss/LossPi', float(loss_pi.item()), self.total_train_steps)
        # Finally, update target networks by polyak averaging.
        with torch.no_grad():
            for p, p_targ in zip(self.q1_net.parameters(), self.q1_net_targ.parameters()):
                # NB: We use an in-place operations "mul_", "add_" to update target
    ...
    def test_agent(self):
        ...
        self.writer.add_scalar('rew/test_rew', float(total_rewards.mean()), self.total_train_steps)
    def train(self):
        ...
        while self.total_train_steps < total_steps:
            ...
            # Update handling
            if self.total_env_steps >= self.update_begin_steps and self.total_env_steps % 1000 == 0:
                for j in range(200):
                ...
                if self.total_train_steps % self.latent_encoder_update_frequency == 0:  # 服务器
                    ...
                    for j in range(self.latent_encoder_update_frequency):  # 1000
