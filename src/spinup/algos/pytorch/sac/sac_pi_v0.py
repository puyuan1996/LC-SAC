...
class SAC(object):
    def __init__(self, env, env_name, agent, q1_net, q2_net, ac_kwargs=dict(), seed=0,
                 ...
                 , collect_data_samples=10000, latent_encoder_update_frequency=10, logger_kwargs=dict()):
        super().__init__()
    ...
    def update_step(self, context_batch_indices, context_seq_batch, update_latent_encoder):
        ...
        #     1 + 2 * self.agent.z_log_std - self.agent.z_mu.pow(2) - self.agent.z_log_std.exp().pow(2))
    ...
    def collect_data(self, nums_steps, add_to_latent_buffer=True, deterministic=False):
        '''
    ...
    def train(self):
        ...
        while self.total_train_steps < total_steps:
            ...
            #     self.rl_replay_buffer.clear()
            self.collect_data(self.collect_data_samples, add_to_latent_buffer=True, deterministic=False)  # 5000
            for train_step in range(self.train_steps):  # 500
                # if train_step > 1e5 and train_step % self.latent_encoder_update_frequency == 0: # 本机
                if self.total_train_steps % self.latent_encoder_update_frequency == 0:  # 服务器
                    self.update_latent_encoder = True
                self.train_step_new(self.update_latent_encoder)
                self.update_latent_encoder = False
