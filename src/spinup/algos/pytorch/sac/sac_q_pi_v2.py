...
class SAC(object):
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch, update_latent_encoder):
        ...
        # data = data.to(self.device)
        z = self.agent(context_seq_batch.to(self.device))
        if self.total_latent_train_steps%5==0:
            # KL constraint on z if probabilistic
            if update_latent_encoder == True:
                self.latent_optimizer.zero_grad()
            # First run one gradient descent step for Q1 and Q2
    ...
    def train(self):
        ...
        while self.total_train_steps < total_steps:
            ...
            if self.total_env_steps % 2000 == 0:
                ...
                print(f'recent 2000 steps ep_ret mean max min std :', np.array(ep_ret_list).mean(),
            ...
            # Update handling
            if self.total_env_steps >= self.update_begin_steps and self.total_env_steps % 400 == 0:
                for j in range(50):
