...
class SAC(object):
    def __init__(self, env, env_name, agent, q1_net, q2_net, seed=0,
        ...
        self.epochs = epochs
        # self.rl_buffer_size_q = args.rl_buffer_size_q
        # self.rl_buffer_size_pi = args.rl_buffer_size_pi
        self.rl_buffer_size_q = 1000000
        self.rl_buffer_size_pi = args.latent_encoder_update_every #TODO
        self.gamma = gamma
        self.polyak = polyak
        self.lr = lr
        ...
        # prefix = './result_cpl_noQLoss_meta_ml1_train1_test1/'  # TODO
        prefix = './result_cpl_sar_noQLoss_ml1_train1_test1_train_z_det/' if self.z_deterministic else './result_cpl_sar_noQLoss_ml1_train1_test1_train_z_prob/'  # TODO
        self.model_path = prefix + f'model_cpl_v3/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}/{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_every}_{self.rl_update_every}_' \
                                   f'{self.latent_buffer_size}_{self.rl_buffer_size_q}/'
        ...
        self.writer_prefix = f'{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}_{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_every}_{self.rl_update_every}_' \
                             f'{self.latent_buffer_size}_{self.rl_buffer_size_q}_'
        ...
        # Experience buffer
        self.rl_replay_buffer_q = ReplayBufferZ(latent_dim=args.latent_dim, obs_dim=obs_dim, act_dim=act_dim,
                                              size=self.rl_buffer_size_q)
        self.rl_replay_buffer_pi = ReplayBufferZ(latent_dim=args.latent_dim, obs_dim=obs_dim, act_dim=act_dim,
                                              size=self.rl_buffer_size_pi)
        ...
        self.contrast_encoder = contrast(act=False, z_dim=self.latent_dim, obs_dim=self.env.observation_space.shape[0]+self.env.action_space.shape[0]+1,
    ...
    def update_step_rl_q(self, data, z2):
    ...
    def update_step_rl_pi(self, data):
        z = data['z'].to(self.device)
        data = {k: v for k, v in data.items() if k != 'z'}  # and k != 'z2'}
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch):
        ...
        loss_cpl = self.contrast_encoder.loss_cpl(z[1], data)  # TODO z[1]
    ...
    def train_step_rl_q(self):
        data, indices = self.rl_replay_buffer_q.random_batch(self.latent_batch_size)
        data2 = self.rl_replay_buffer_q.sample_data(indices + 1)
        z2 = data2['z']
        self.update_step_rl_q(data, z2)  # sac rl update

    def train_step_rl_pi(self):
        data, indices = self.rl_replay_buffer_pi.random_batch(self.latent_batch_size)
        self.update_step_rl_pi(data)  # sac rl update
    ...
    def train(self):
        ...
        while self.total_train_steps < total_steps:
            ...
            self.rl_replay_buffer_q.add_sample(o, a, r, o2, d, z)
            self.rl_replay_buffer_pi.add_sample(o, a, r, o2, d, z)
            ...
            if self.total_env_steps >= self.update_begin_steps and self.total_env_steps % self.latent_encoder_update_every == 0:  # 服务器
                ...
                for j in range(self.latent_encoder_update_every // self.latent_fq):  # 5000//1=5000
                    self.train_step_rl_pi()  # TODO
