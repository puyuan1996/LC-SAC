...
class SAC(object):
    def __init__(self, env, env_name, agent, q1_net, q2_net, seed=0,
        ...
        self.model_path = f'model_v2/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}_k{args.kl_lambda}/{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_frequency}_{self.rl_update_frequency}_' \
                          f'{args.latent_buffer_size}_{args.rl_buffer_size}/'

        self.writer = SummaryWriter(f'tblogs_v2/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}_k{args.kl_lambda}/{args.latent_fq}_{args.rl_fq}_'
    ...
    def compute_loss_pi(self, data, z):
        ...
        pi_action, logp_pi, policy_mean, policy_log_std = self.agent.pi(o_z, deterministic=False, with_logprob=True) # TODO
    ...
    def update_step_rl(self, context_batch_indices, context_seq_batch, update_latent_encoder):
        ...
        z, _, _ = self.agent(context_seq_batch.to(self.device))

        # First run one gradient descent step for Q1 and Q2
        self.q_optimizer.zero_grad()
        loss_q = self.compute_loss_q(data, z.detach())
        ...
        loss_pi = self.compute_loss_pi(data, z[0].detach())
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch, update_latent_encoder):
        ...
        # First run one gradient descent step for Q1 and Q2
        # self.q_optimizer.zero_grad()
        loss_q = self.compute_loss_q(data, z)
