...
class SAC(object):
    def __init__(self, env, env_name, agent, q1_net, q2_net, seed=0,
        ...
        self.model_path = f'model_v4/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}_k{args.kl_lambda}/{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_frequency}_{self.rl_update_frequency}_' \
                          f'{args.latent_buffer_size}_{args.rl_buffer_size}/'
        self.test_rew_path = f'test_rew_v4/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}_k{args.kl_lambda}/{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_frequency}_{self.rl_update_frequency}_' \
                             f'{args.latent_buffer_size}_{args.rl_buffer_size}/'
        self.writer_prefix = f'{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}_k{args.kl_lambda}_{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_frequency}_{self.rl_update_frequency}_' \
                             f'{args.latent_buffer_size}_{args.rl_buffer_size}_'
        self.writer = SummaryWriter(
            f'tblogs_v4/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}/{args.latent_fq}_{args.rl_fq}_'
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch):
        ...
        for p in self.agent.pi.parameters():
            p.requires_grad = False
        for i in range(self.seq_len // 2):  # 8//2=4
            data = self.latent_replay_buffer.sample_data(np.array(context_batch_indices)[:, i])

            # see Appendix B from VAE paper:
            # Kingma and Welling. Auto-Encoding Variational Bayes. ICLR, 2014
            # https://arxiv.org/abs/1312.6114
            # 0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
            KLD = -0.5 * torch.sum(1 + 2 * self.z_log_std[:, 2 * i, :] - self.z_mu[:, 2 * i, :].pow(2) - (
                        2 * self.z_log_std[:, 2 * i, :]).exp())

            # First run one gradient descent step for Q1 and Q2
            # self.q_optimizer.zero_grad()
            # loss_q = self.compute_loss_q(data, z[0],z[1])
            loss_q = self.compute_loss_q(data, z[:, 2 * i, :], z[:, 2 * i + 1, :].detach())  # TODO
            loss_latent_1 = loss_q + self.kl_lambda * KLD
            loss_latents = loss_latents + loss_latent_1

        loss_latents.backward()
        # self.q_optimizer.step()
    ...
    def train_step_latent(self):
        ...
        for i in range(self.latent_batch_size):  # 随机采集100个长度为seq_len的sequence
            ...
            # context_batch_indices.append([indices[-7], indices[-5], indices[-3], indices[-1]])
            # context_batch_indices.append([indices[i] for i in range(-self.seq_len + 1, 0, 2)])
            context_batch_indices.append([indices[i] for i in range(1, self.seq_len , 2)])
