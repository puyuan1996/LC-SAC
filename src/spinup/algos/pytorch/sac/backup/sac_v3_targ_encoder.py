...
class SAC(object):
    def __init__(self, env, env_name, agent, q1_net, q2_net, seed=0,
        ...
        self.model_path = f'model_v3/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}/{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_frequency}_{self.rl_update_frequency}_' \
                          f'{args.latent_buffer_size}_{args.rl_buffer_size}/'
        self.test_rew_path = f'test_rew_v3/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}/{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_frequency}_{self.rl_update_frequency}_' \
                             f'{args.latent_buffer_size}_{args.rl_buffer_size}/'
        ...
        self.writer = SummaryWriter(
            f'tblogs_v3/{self.env_name}_s{self.seed}_l{args.seq_len}_d{args.latent_dim}/{args.latent_fq}_{args.rl_fq}_'
        ...
        self.agent_targ = deepcopy(agent)
        ...
        for p in self.q2_net_targ.parameters():
            p.requires_grad = False
        for p in self.agent_targ.latent_encoder.parameters():
            p.requires_grad = False
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch, update_latent_encoder):
        ...
        z_targ, _, _ = self.agent_targ(context_seq_batch.to(self.device))
        ...
        loss_q = self.compute_loss_q(data, z[0], z_targ[1].detach())  # TODO
        ...
        with torch.no_grad():
            for p, p_targ in zip(self.agent.latent_encoder.parameters(), self.agent_targ.latent_encoder.parameters()):
                # NB: We use an in-place operations "mul_", "add_" to update target
                # params, as opposed to "mul" and "add", which would make new tensors.
                p_targ.data.mul_(self.polyak)
                p_targ.data.add_((1 - self.polyak) * p.data)
