...
class SAC(object):
    def __init__(self, env, env_name, agent, q1_net, q2_net, seed=0,
        ...
        self.model_path = f'model_z_tcl/{self.env_name}_s{self.seed}/{args.latent_fq}_{args.rl_fq}_{self.latent_encoder_update_frequency}_{self.rl_update_frequency}_' \
                          f'{args.latent_buffer_size}_{args.rl_buffer_size}/'

        self.writer = SummaryWriter(f'tblogs_z_tcl/{self.env_name}_s{self.seed}/{args.latent_fq}_{args.rl_fq}_'
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch, update_latent_encoder):
        ...
        loss_tcl = self.contrast_encoder.loss_tcl(z[1], data)
