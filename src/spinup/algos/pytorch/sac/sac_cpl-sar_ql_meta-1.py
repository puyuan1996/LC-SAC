...
class SAC(object):
    def __init__(self, env, env_name, agent, q1_net, q2_net, seed=0,
        ...
        self.ql_lambda =  args.ql_lambda
        prefix=f'./result_cplsar_ql-{self.ql_lambda}-{self.kl_lambda}_meta_ml1_train1_test1_train_z'
        prefix += '_det' if self.z_deterministic else '_prob'  # TODO
    ...
    def update_step_latent(self, context_batch_indices, context_seq_batch):
        ...
        loss_latent = self.cpl_lambda * loss_cpl + self.ql_lambda * loss_q + self.kl_lambda * KLD  # TODO
